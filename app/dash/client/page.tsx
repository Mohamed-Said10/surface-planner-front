...
export default function HomePage() {
  ...
  const fetchStatusHistory = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/bookings/status-history/last`, {
        credentials: "include",
        headers: { "Content-Type": "application/json" },
        signal,
      })
    ...
    const loadBookings = async () => {
      try {
        ...
        setStats({
          ...
          activeBookings: data.bookings.filter((b: any) => !["COMPLETED", "CANCELLED"].includes(b.status)).length,
        })
      } catch (err) {
